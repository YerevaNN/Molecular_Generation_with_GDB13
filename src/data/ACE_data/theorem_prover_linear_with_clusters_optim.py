...
def worker_fn_for_batch_multi(pairs, prover_path, timeout, show_progress, indx):
    results = []

    for t in pairs:
        # Process each pair as before
        results.append(worker_fn_for_batch(t, prover_path, timeout, show_progress, indx))
...
def check_equivalence_batch_optimized(path_batch, axiom_line_batch, conj_line, executor, prover_path, timeout, show_progress, indx):
    ...
    futures = [
        executor.submit(worker_fn_for_batch_multi, t, prover_path, timeout, show_progress, indx)
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=num_threads) as thread_executor:
        for indx, conj_line in tqdm(enumerate(conjs_1), disable=not show_progress):
            ...
            for batch in clusters_batches:
                ...
                equivalence_result, cluster_path = check_equivalence_batch_optimized(cluster_path_batch, cluster_tptp_batch, conj_line, thread_executor, prover_path, timeout, show_progress, indx)
